	...
	for (i = 0; i < nx; i++) {
		for (j = 0; j < ny; j++) {
			for (k = 0; k < nz+pad*2; k++) {
				...
				if (pad <= k && k < nz/4 + pad) media_id(i, j, k) = 0;
				else if (k < 2*nz/4 + pad) media_id(i, j, k) = 1;
				else if (k < 3*nz/4 + pad) media_id(i, j, k) = 0;
				else if (k < nz + pad) media_id(i, j, k) = 1;

				if (k == pad) rest = nz/4;
				else if (k == nz/4 + pad) rest = nz/4;
				else if (k == 2*nz/4 + pad) rest = nz/4;
				else if (k == 3*nz/4 + pad) rest = nz/4;
