...
// This program uses `gather` instruction to load media constants.
...
void init() {
	...
	media_id = _mm_malloc((nx) * (ny) * (nz + pad * 2) * sizeof(int), 64);
	...
	for (i = 1; i < nx-1; i++) {
		for (j = 1; j < ny-1; j++) {
			for (k = pad; k < nz+pad; k += 8) {
				...
				__m512d fgh = zcexrz * _mm512_loadu_pd(&hy(i, j, k-1));
				...
				cde = zceyrz * _mm512_loadu_pd(&hx(i, j, k-1));
	...
	for (i = 1; i < nx-1; i++) {
		for (j = 1; j < ny-1; j++) {
			for (k = pad; k < nz+pad; k += 8) {
				...
				__m512d efg = _mm512_fmsub_pd(zchxrz, _mm512_loadu_pd(&ey(i, j, k+1)), zchxrz * ey0);
				_mm512_store_pd(&hx(i, j, k), _mm512_load_pd(&hx(i, j, k)) - bcd + efg);

				//hy(i, j, k) = hy(i, j, k)
				//	- chyrz0 * (ex(i, j, k+1) - ex(i, j, k))
				//	+ chyrx0 * (ez(i+1, j, k) - ez(i, j, k));
				bcd = _mm512_fmsub_pd(zchyrz, _mm512_loadu_pd(&ex(i, j, k+1)), zchyrz * ex0);
